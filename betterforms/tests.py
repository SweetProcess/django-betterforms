...
class TestUtils(TestCase):
    def test_flatten(self):
        ...
        self.assertTupleEqual(
            flatten_to_tuple(fields3), ("a", "b", "c", "d", "e", "f", "g", "h", "i")
        )
...
class TestBetterForm(TestCase):
    ...
    def test_form_error(self):
        ...
        self.assertFalse(form.is_valid())
        self.assertDictEqual(form.errors, {"__all__": ["test"]})
...
class TestChangleListQuerySetAPI(TestCase):
    ...
    def test_with_model_declaration_and_provided_queryset(self):
        form = self.TestChangeListForm(
            {"foo": "arst"},
            queryset=ChangeListModel.objects.exclude(field_a="0").exclude(field_a="1"),
        )
...
class TestSearchFormAPI(TestCase):
    ...
    @unittest.skipIf(
        ...
        "Case Sensitive __contains queries are not supported on sqlite.",
    )
...
class TestHeaderSetAPI(TestCase):
    ...
    def test_iteration_yields_bound_headers(self):
        ...
        self.assertTrue(all((isinstance(header, BoundHeader) for header in header_set)))
...
class TestSortFormAPI(TestCase):
    ...
    def test_sort_field_cleaning(self):
        ...
        self.assertIn(
            self.TestSortForm.error_messages["unsortable_header"],
            unsortable.errors["sorts"],
        )
        ...
        self.assertIn("sorts", unsortable.errors)
        self.assertIn(
            self.TestSortForm.error_messages["unknown_header"],
            unsortable.errors["sorts"],
        )
